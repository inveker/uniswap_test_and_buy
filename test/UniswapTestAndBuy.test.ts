...
import TEST_TOKENS from '../python_integration/input.json';
...
const PATH_OUTPUT = path.join(__dirname, '../python_integration/output.json');
...
describe(`UniswapTestAndBuy.sol`, () => {
    ...
    for (const TOKEN of TEST_TOKENS) {
        it(`Regular test ${TOKEN}`, async () => {
            try {
                ...
                outputData.true.push(TOKEN);
            } catch(e) {
                outputData.false.push(TOKEN);
                throw e;
            }
        });
    }
    ...
    // --------------------------------------------------------
    // -------------  AFTER, SAVE DATA TO FILE  ---------------
    // --------------------------------------------------------

    after(async () => {
        fs.writeFileSync(PATH_OUTPUT, JSON.stringify(outputData));
    });
});